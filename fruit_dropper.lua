...
-- - Webhook send
-- - Drop by Value (delivery mode)
-- ============================================================
if game:GetService("CoreGui"):FindFirstChild("GAG_AltPicker") then
	game:GetService("CoreGui").GAG_AltPicker:Destroy()
end
...
-- =====================
local WEBHOOK_URL =
	"https://discord.com/api/webhooks/1520692534135750778/x3d9XAM0Zp8j2MwB6D1EqYumKfZYAYjMaEIQnw2zArT2jtY7EN0Mdp6ZsvgQIpj7VOPh"
...
main.Position = UDim2.new(1, -360, 0.5, -270)
...
mkLabel(hdr, "🎒 ALT PICKER V5", UDim2.new(1, -60, 1, 0), UDim2.new(0, 12, 0, 0), C.accent, 12, true)
...
local function doScan(favouriteFilter, btn, label)
	...
	for i, entry in ipairs(fruits) do
		-- hard timing gate (FPS-proof)
		local now = os.clock()
		if now < nextTime then
			task.wait(nextTime - now)
		end
		nextTime = os.clock() + SCAN_INTERVAL

		btn.Text = string.format("Scanning %d/%d...", i, total)
...
scanDeliveryBtn.MouseButton1Click:Connect(function()
	...
	-- Reset locked button appearance
...
scanLockedBtn.MouseButton1Click:Connect(function()
	...
	-- Reset delivery button appearance
...
-- Default overshoot allowance for target mode; overridden per-input with "~"
local DEFAULT_ALLOWED_THRESHOLD = 3

-- Supported dropValInput formats:
--   "50"        -> target mode: drop fruit totaling ~50, largest value first
--                  (default overshoot allowance: DEFAULT_ALLOWED_THRESHOLD)
--   "50~5"      -> same, but allow up to 5 overshoot instead of the default
--   ">10"       -> drop every fruit valued > 10
--   "<10"       -> drop every fruit valued < 10
--   ">=10"      -> drop every fruit valued >= 10
--   "<=10"      -> drop every fruit valued <= 10
--   ">10<50"    -> drop every fruit valued between 10 and 50 (exclusive)
--   "50<10"     -> target mode, only consider fruit valued < 10
--   "50~5<10"   -> target mode, 5 overshoot allowed, only fruit valued < 10
--   "50>=10"    -> target mode, only consider fruit valued >= 10 (etc.)

-- Parses a standalone comparison filter: ">X", "<X", ">=X", "<=X", ">min<max"
local function parseComparison(text)
	...
	if n then
		return { type = "gte", value = tonumber(n) }
	end

	n = text:match("^<=([%d%.]+)$")
	if n then
		return { type = "lte", value = tonumber(n) }
	end

	n = text:match("^>([%d%.]+)$")
	if n then
		return { type = "gt", value = tonumber(n) }
	end

	n = text:match("^<([%d%.]+)$")
	if n then
		return { type = "lt", value = tonumber(n) }
	end
...
local function parseDropInput(text)
	...
	-- Leading number = target mode, optionally followed by "~<threshold>"
	-- and/or a trailing comparison filter (parsed via parseComparison).
	local targetStr, thresholdStr, rest = text:match("^([%d%.]+)~([%d%.]+)(.*)$")

	if not targetStr then
		targetStr, rest = text:match("^([%d%.]+)(.-)$")
	end
	...
	-- No leading number -> standalone comparison filter
	return parseComparison(text)
end

local function matchesFilter(value, filter)
	if filter.type == "gt" then
		return value > filter.value
	elseif filter.type == "lt" then
		return value < filter.value
	elseif filter.type == "gte" then
		return value >= filter.value
	elseif filter.type == "lte" then
		return value <= filter.value
	elseif filter.type == "range" then
		return value > filter.min and value < filter.max
	end
...
dropValBtn.MouseButton1Click:Connect(function()
	...
	if #queue == 0 then
		dropValStatusLbl.Text = "⚠️ No matching fruits"
		return
	end
	...
	if filter.type == "target" then
		...
		if total < filter.value then
			if available < filter.value then
				dropValStatusLbl.Text =
					string.format("⚠️ Only %s available (need %dM)", formatVal(available), filter.value)
			else
				dropValStatusLbl.Text = string.format("⚠️ Closest combination: %s", formatVal(total))
			end
			return
		end
	end
	...
	task.spawn(function()
		...
		local function dropEntry(entry)
			if filter.type == "target" then
				dropValStatusLbl.Text = string.format("Dropping %d/%d · %s", dropped + 1, #queue, formatVal(total))
			else
				dropValStatusLbl.Text = string.format("Dropping %d · %s", dropped + 1, formatVal(total))
			end
			...
			if batchCount >= BATCH_SIZE then
				...
				while droppingByVal and tick() - waitStart < 60 do
					...
					if di and #di:GetChildren() == 0 then
						break
					end
		...
		dropValStatusLbl.Text = string.format("✓ Dropped %d fruits", dropped)
	end)
end)
...
webhookBtn.MouseButton1Click:Connect(function()
	...
	for _, e in ipairs(backpackFruits) do
		...
		local tierLabel = e.tierIdx and TIERS[e.tierIdx].label or "Unknown"
		...
		table.insert(
			...
			string.format(
				...
				fav,
				e.name,
				e.mutation ~= "" and e.mutation or "None",
				e.kg,
				formatVal(e.value)
			)
		)
