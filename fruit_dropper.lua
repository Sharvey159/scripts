...
mkLabel(hdr, "🎒 ALT PICKER V3", UDim2.new(1, -60, 1, 0), UDim2.new(0, 12, 0, 0), C.accent, 12, true)
...
local function doScan(favouriteFilter, btn, label)
	...
	for i, entry in ipairs(fruits) do
		...
		entry.tierIdx = getTierIndex(entry.value)
		task.wait(0.1)
	end
...
local function parseDropInput(text)
	...
	-- Plain number = original target mode
...
webhookBtn.MouseButton1Click:Connect(function()
	...
	for _, e in ipairs(backpackFruits) do
		total = total + e.value
		local tierLabel = e.tierIdx and TIERS[e.tierIdx].label or "?"
	...
	local footer = "\n━━━━━━━━━━━━━━━━━━━━━━\n"
		.. string.format("**ALT %s: %d fruits · %s**", modeTag, #backpackFruits, formatVal(total))
		.. "\n"
